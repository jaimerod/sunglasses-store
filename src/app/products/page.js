...
export default function ProductsPage() {
  ...
  // Parse filters and search from URL query params
  ...
  // Update URL when filters or debounced search change
  useEffect(() => {
  ...
  function clearAll() {
    setFilters(
      Object.keys(FACETS).reduce((acc, f) => {
        acc[f] = new Set();
        return acc;
      }, {})
    );
  ...
  // Filter products by facets and search text
  const filteredProducts = useMemo(() => {
    const lowerSearch = search.toLowerCase();
    return products.filter((product) => {
      // Search filter
      if (search && !product.name.toLowerCase().includes(lowerSearch)) {
        return false;
      }
      // Facets filter
  ...
  // Infinite scroll: slice filtered products by page size
  ...
  return (
    <div className="flex min-h-screen p-6 gap-8 bg-background text-foreground">
      {/* Sidebar filters */}
      <aside className="w-64 border-r border-gray-300 dark:border-gray-700 pr-4 sticky top-24 self-start">
        ...
        <div className="mb-4">
          <input
            ...
            className="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring focus:border-blue-500 dark:bg-gray-800 dark:border-gray-600"
          />
        </div>
      ...
      <section className="flex-1 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <div className="mt-3 text-xs text-gray-500 space-y-1">
                    ...
                    <p>
                      <strong>Frame Color:</strong> {product.frameColor}
                    </p>
                    <p>
                      <strong>Frame Material:</strong> {product.frameMaterial}
                    </p>
                    <p>
                      <strong>Lens Color:</strong> {product.lensColor}
                    </p>
                    <p>
        ...
        {/* Loader if more products to load */}
        {displayedProducts.length < filteredProducts.length && (
